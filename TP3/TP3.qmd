...
# Features are generated from standard normal distribution
X <- matrix(rnorm(n * p), nrow = n, ncol = p)
colnames(X) <- paste0("X", 1:p)

# Define true beta coefficients
# Some coefficients are exactly zero to create sparse model
# This simulates real-world scenario where not all features are relevant

beta_true <- c(
  3.5,   # X1  - Strong positive effect
  -2.8,  # X2  - Strong negative effect
  1.5,   # X3  - Moderate positive effect
  0,     # X4  - Zero (irrelevant)
  0,     # X5  - Zero (irrelevant)
  2.2,   # X6  - Moderate positive effect
  0,     # X7  - Zero (irrelevant)
  -1.8,  # X8  - Moderate negative effect
  0,     # X9  - Zero (irrelevant)
  0,     # X10 - Zero (irrelevant)
  1.2,   # X11 - Small positive effect
  0,     # X12 - Zero (irrelevant)
  0,     # X13 - Zero (irrelevant)
  0,     # X14 - Zero (irrelevant)
  -0.9,  # X15 - Small negative effect
  0,     # X16 - Zero (irrelevant)
  0,     # X17 - Zero (irrelevant)
  0,     # X18 - Zero (irrelevant)
  0,     # X19 - Zero (irrelevant)
  0      # X20 - Zero (irrelevant)
)
```

```{r}
# Count non-zero coefficients (true model size)
true_model_size <- sum(beta_true != 0)

cat("True Beta Coefficients:\n")
beta_df <- data.frame(
  Feature = paste0("X", 1:p),
  Coefficient = beta_true,
  Status = ifelse(beta_true == 0, "Zero (Irrelevant)", "Non-Zero (Relevant)")
)
print(beta_df)

cat("\n\nTrue Model Characteristics:\n")
cat(sprintf("- Number of relevant features: %d\n", true_model_size))
cat(sprintf("- Number of irrelevant features: %d\n", p - true_model_size))
cat(sprintf("- True model includes:  %s\n", 
            paste(paste0("X", which(beta_true != 0)), collapse = ", ")))
```

```{r}
# Generate error term
# Using standard normal errors
epsilon <- rnorm(n, mean = 0, sd = 2)

# Generate response variable Y = Xβ + ε
Y <- X %*% beta_true + epsilon

# Calculate theoretical R²
signal_variance <- var(X %*% beta_true)
noise_variance <- var(epsilon)
theoretical_R2 <- signal_variance / (signal_variance + noise_variance)

cat(sprintf("\nTheoretical R² (population): %.4f\n", theoretical_R2))
...
**Why Calculate Theoretical R² and SNR? To verify Data Generation Quality**

1.  **R² = 0.88**

    -   88% of Y's variance comes from true predictors (X₁, X₂, X₃, X₆, X₈, X₁₁, X₁₅)

    -   12% comes from random noise

2.  **SNR = 7.65**

    -   Signal is \~7.6 times larger than noise

    -   This means:

        -   Strong effects (like β₁ = 3. 5) will definitely be detected

        -   Weak effects (like β₁₅ = -0.9) might be borderline

        -   Noise variables won't accidentally look important
...
cat(sprintf("Training set size: %d observations\n", n_train))
...
```{r}
# Perform best subset selection
# This fits all possible models and keeps the best model of each size
regfit_full <- regsubsets(Y ~ ., data = train_data, nvmax = p, method = "exhaustive")

# Get summary
reg_summary <- summary(regfit_full)

cat("Best Subset Selection Complete\n")
cat(sprintf("Models fitted: %d (one for each size from 1 to %d variables)\n", p, p))

reg_summary
```

**Variables Selected (from your output):**

| **Model Size** | **Variables Selected**                                    |
|----------------|-----------------------------------------------------------|
| 1              | **X1** (strongest predictor, β=3.5)                       |
| 2              | **X1, X2** (β=3.5, -2.8)                                  |
| 3              | **X1, X2, X6** (adding another strong predictor)          |
| 4              | **X1, X2, X6, X8**                                        |
| 5              | **X1, X2, X3, X6, X8**                                    |
| 6              | **X1, X2, X3, X6, X8, X11**                               |
| 7              | **X1, X2, X3, X6, X8, X11, X15** (all true predictors!) ✓ |
| 8-20           | Gradually adds noise variables (X4, X5, X7, etc.)         |

**Note**: By size 7, the model has captured all true predictors:

-   True non-zero: X1, X2, X3, X6, X8, X11, X15

-   Selected at size 7: X1, X2, X3, X6, X8, X11, X15 (Perfect Match)

This demonstrates that the **last 13 variables add very little to training fit**, yet they are **all noise variables**. This is the essence of overfitting that Part 4 will expose.
...
for (i in 1:p) {
  ...
  y_train_pred <- predict.regsubsets(regfit_full, train_data, id = i)
  
  # Calculate training MSE
  train_mse[i] <- mean((train_data$Y - y_train_pred)^2)
}

cat("\n\nTraining Set MSE for Each Model Size:\n")
train_mse_df <- data.frame(
  Model_Size = 1:p,
  Training_MSE = train_mse,
  Training_RMSE = sqrt(train_mse)
)
print(train_mse_df)

cat(sprintf("\nMinimum Training MSE: %.4f (Model Size:  %d)\n", 
            min(train_mse), which.min(train_mse)))
```

```{r}
p1 <- ggplot(data.frame(Model_Size = 1:p, MSE = train_mse), 
             aes(x = Model_Size, y = MSE)) +
  geom_line(color = "blue", size = 1.2) +
  geom_point(color = "blue", size = 3) +
  geom_point(aes(x = which.min(train_mse), y = min(train_mse)), 
             color = "red", size = 5, shape = 18) +
  geom_vline(xintercept = true_model_size, linetype = "dashed", 
             color = "darkgreen", size = 1) +
  annotate("text", x = true_model_size, y = max(train_mse), 
           label = paste("True Model\nSize =", true_model_size), 
           hjust = -0.1, color = "darkgreen", fontface = "bold") +
  labs(title = "Part 3: Training Set MSE vs Model Size",
       subtitle = "Best subset selection on training set",
...
The plot shows the **training set MSE** for the best model of each size (1 to 20 variables) obtained through best subset selection.

**Key Observations:**

1.  **Monotonic Decrease**: Training MSE decreases consistently as more variables are added to the model

    -   1 variable: MSE ≈ 21.5

    -   7 variables (true model size): MSE ≈ 3.5

    -   20 variables (full model): MSE ≈ 2.8 (minimum, marked with red diamond)

2.  **Steep Initial Decline**: The most dramatic improvement occurs in the first few variables:

    -   Adding X1 (1st variable): MSE drops from \~21.5 to \~12.5

    -   Adding X2 (2nd variable): MSE drops to \~8.8

    -   Adding X6 (3rd variable): MSE drops to \~6.7

3.  **Diminishing Returns**: After \~7 variables, improvements become minimal:

    -   Variables 7-20 only reduce MSE by \~0.7 (from 3.5 to 2.8)

    -   The curve flattens significantly

4.  **True Model Size (Green Dashed Line)**:

    -   Marked at 7 variables (the true number of non-zero coefficients)

    -   At this point, training MSE ≈ 3.3

    -   Additional 13 variables only reduce MSE by 15%
...
for (i in 1:p) {
  # Predict on test set
  y_test_pred <- predict.regsubsets(regfit_full, test_data, id = i)
  
  # Calculate test MSE
  test_mse[i] <- mean((test_data$Y - y_test_pred)^2)
}

cat("Test Set MSE for Each Model Size:\n")
comparison_df <- data.frame(
  Model_Size = 1:p,
  Training_MSE = train_mse,
  Test_MSE = test_mse,
  Difference = test_mse - train_mse,
  Ratio = test_mse / train_mse
)
print(comparison_df)

cat(sprintf("\nMinimum Test MSE: %.4f (Model Size: %d)\n", 
            min(test_mse), which.min(test_mse)))
cat(sprintf("True Model Size: %d\n", true_model_size))
```

```{r}
# Prepare data for plotting
plot_data <- data.frame(
  Model_Size = rep(1:p, 2),
  MSE = c(train_mse, test_mse),
  Dataset = rep(c("Training", "Test"), each = p)
)

# Plot 1: Training MSE only (Part 3)
p1 <- ggplot(data.frame(Model_Size = 1:p, MSE = train_mse), 
             aes(x = Model_Size, y = MSE)) +
  geom_line(color = "blue", size = 1.2) +
  geom_point(color = "blue", size = 3) +
...
# Plot 2: Test MSE only (Part 4)
p2 <- ggplot(data.frame(Model_Size = 1:p, MSE = test_mse), 
             aes(x = Model_Size, y = MSE)) +
  geom_line(color = "red", size = 1.2) +
  geom_point(color = "red", size = 3) +
  geom_point(aes(x = which.min(test_mse), y = min(test_mse)), 
             color = "darkred", size = 5, shape = 18) +
  geom_vline(xintercept = true_model_size, linetype = "dashed", 
             color = "darkgreen", size = 1) +
  annotate("text", x = true_model_size, y = max(test_mse), 
           label = paste("True Model\nSize =", true_model_size), 
           hjust = -0.1, color = "darkgreen", fontface = "bold") +
  labs(title = "Part 4: Test Set MSE vs Model Size",
...
# Plot 3: Both training and test MSE (Combined view)
p3 <- ggplot(plot_data, aes(x = Model_Size, y = MSE, color = Dataset, group = Dataset)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  geom_point(data = data.frame(Model_Size = which.min(train_mse), 
                                MSE = min(train_mse), 
                                Dataset = "Training"),
             size = 5, shape = 18, color = "blue") +
  geom_point(data = data.frame(Model_Size = which.min(test_mse), 
                                MSE = min(test_mse), 
                                Dataset = "Test"),
             size = 5, shape = 18, color = "red") +
  geom_vline(xintercept = true_model_size, linetype = "dashed", 
             color = "darkgreen", size = 1) +
  annotate("text", x = true_model_size, y = max(c(train_mse, test_mse)), 
           label = paste("True Model Size =", true_model_size), 
           hjust = -0.1, color = "darkgreen", fontface = "bold", size = 4) +
  scale_color_manual(values = c("Training" = "blue", "Test" = "red")) +
  labs(title = "Training vs Test Set MSE",
       subtitle = sprintf("Minimum Test MSE at %d variables (True model:  %d variables)", 
                         which.min(test_mse), true_model_size),
       x = "Number of Variables in Model",
...
# Plot 4: Difference between test and training MSE (Overfitting)
p4 <- ggplot(data.frame(Model_Size = 1:p, Difference = test_mse - train_mse), 
             aes(x = Model_Size, y = Difference)) +
  geom_line(color = "purple", size = 1.2) +
  geom_point(color = "purple", size = 3) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  geom_vline(xintercept = true_model_size, linetype = "dashed", 
             color = "darkgreen", size = 1) +
  labs(title = "Generalization Gap: Test MSE - Training MSE",
       subtitle = "Positive values indicate overfitting",
       x = "Number of Variables in Model",
       y = "Test MSE - Training MSE") +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold", size = 14),
        axis.title = element_text(size = 12))
...
cat("Optimal size:", optimal_size)
cat("\nOptimal test MSE:", optimal_test_mse)
```
...
Suppose you have a dataset shipping data.csv containing n= 1500 deliveries from a global online retailer. The goal is to predict the total Transit Time (the number of days from dispatch to arrival) using various shipment characteristics. The dataset includes the following variables:
...
\(b\) Use R to calculate the 99% confidence intervals for all regression coefficients. Based on these intervals, determine which predictors have a statistically significant relationship with shipping speed at the α= 0.01 level.

\(c\) Reproduce the least squares estimate of the error variance σ2. Hint: Use the residuals() function to obtain the residuals ei, and calculateˆσ2=e2 in−p−1.

\(d\) Use Rto estimate both the coefficient of determination R2 and the adjusted R2 adj . Compare these values with the output provided in the model summary from part (a).
...
\(f\) Obtain the R2 adj from the reduced model identified in (e) and compare it to the full model from (a). Discuss whether the reduction in variables significantly impacted the model’s explanatory power.
...
\(i\) Test whether it is useful to add a quadratic polynomial of Weight (i.e., Weight2) to the model. Perform a partial F-test to compare the model with and without the quadratic term
...
## Exercise 2: National Football League Team Performance (1976)
...
• `x3`: Punting average (yards per punt)

• `x4`: Field goal percentage (field goals made divided by field goals attempted)

• `x5`: Turnover differential (turnovers gained minus turnovers lost)
...
library(leaps)     # For all possible regressions
library(car)       # For VIF and other diagnostics
library(tidyverse) # For data manipulation and visualization

# Read the NFL 1976 dataset from CSV file
nfl_data <- read.csv("~/Documents/I4-AMS-B/Statistical-Modeling/statistical-modeling-tp/TP3/NFL_1976_Team_Performance.csv", header = TRUE)

# Display the dataset
cat("NFL 1976 Dataset:\n")
print(nfl_data)
...
# Summary statistics
cat("\n\nSummary Statistics:\n")
summary(nfl_data[, -1])  # Exclude team names

# Correlation matrix
cat("\n\nCorrelation Matrix:\n")
cor_matrix <- cor(nfl_data[, -1])
print(round(cor_matrix, 3))
```

```{r}
# Start with null model
null_model <- lm(y ~ 1, data = nfl_data)
full_model <- lm(y ~ x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9, data = nfl_data)
...
cat("\n\nForward Selection - Selected Variables:\n")
forward_vars <- names(coef(forward_model))[-1]
print(forward_vars)

cat("\n\nForward Selection - Model Formula:\n")
...
4\. Apply **all possible regressions** to the data. For each candidate model, evaluate the criteria $R^2_p$, Mallows’ $C_p$, and the mean square of the residuals (MSRes). Based on these measures, recommend a final subset regression model.
